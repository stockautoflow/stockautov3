...
class DynamicStrategy(bt.Strategy):
    ...
    def _send_notification(self, subject, body):
        try:
            self.log(f"メール通知スレッドを開始: {subject}")
    ...
    def _evaluate_condition(self, cond):
        ...
        if target_type == 'data':
            target_val = getattr(self.data_feeds[tf], target['value'])[0]
            ...
            target_val_str = f"[{target_val[0]},{target_val[1]}]" if isinstance(target_val, list) else f"[{target_val}]"
    ...
    def _check_entry_conditions(self):
        ...
        self.sl_price = self.data.close[0] - self.risk_per_share if True else self.data.close[0] + self.risk_per_share # Placeholder, will be set in place_order
