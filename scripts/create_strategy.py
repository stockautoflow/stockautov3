...
# ==============================================================================
# ファイル: create_strategy.py
# 説明: 戦略のベース定義とカタログのYAMLファイルを生成します。
# 実行方法: python scripts/create_strategy.py
# ==============================================================================
...
# はじめに
# ==============================================================================
# このドキュメントは、長期(long)、中期(medium)、短期(short)の3つの時間軸をすべて利用し、
# 様々な市場の状況に対応するための30パターンのエントリー戦略アイデア集です。
#
...
- name: 2. EMA + MACD + Stochastic
  ...
  entry_conditions:
    ...
    # 【修正】MACDのパラメータ名をbacktrader標準の`period_me1`, `period_me2`に戻しました。
...
- name: 4. Ichimoku(Proxy) + VWAP + EMAクロス
  ...
  entry_conditions:
    ...
    # 【修正】`btrader_strategy.py`の変更に合わせ、`target`のtypeを`indicator`に変更し、closeとvwapを比較。
...
- name: 5. SMAデュアル + MACD + EMA
  ...
  entry_conditions:
    long:
    #【修正】インジケーター同士の比較を`target: {type: indicator}`で行うように変更
...
if __name__ == '__main__':
    print("--- 戦略定義ファイル (strategy_base.yml, strategy_catalog.yml) の生成を開始します ---")
    create_files(project_files)
    print("--- 戦略定義ファイルの生成が完了しました ---")