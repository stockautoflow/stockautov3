...
import yaml
import os
from tools.component_discovery import discover_components
...
def handle_command(args: argparse.Namespace, config: dict, components: dict):
    ...
    if args.command in ["generate", "g"]:
        ...
        execute_command([python_exec, "-m", config['runnable_modules'][args.component]] + args.extra_args)
        ...
        if args.tool_command == "merge":
            ...
            for i, name in enumerate(components_to_process):
                if name in components:
                    ...
                    return_code = execute_command([python_exec, tool_scripts['merge'], components[name]])
            ...
            execute_command([python_exec, tool_scripts[args.tool_command]])
...
if __name__ == "__main__":
    main()
""",
    # (config/commands.yml と tools/component_discovery.py は変更なしのため省略)
}
