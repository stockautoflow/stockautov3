# **株自動トレードシステム 詳細設計書 v5.0 (リアルタイム機能)**
...
* **クラス**: `DynamicStrategy`
* **責務**:
    * (v4.0から変更なし) `run_realtime.py`から渡された戦略パラメータに基づき、インジケーター計算と**エントリー**条件評価を行う。
    * **(更新)** `__init__`で`live_trading`パラメータを受け取り、決済ロジックを切り替える準備をする。
    * **(更新)** `next()`メソッドで、ポジションの有無に応じて処理を分岐する。
        * **ポジションなし**: エントリー条件を評価する (`_check_entry_conditions`)。
        * **ポジションあり**:
            * `live_trading=True`の場合、クライアントサイド決済ロジック (`_check_live_exit_conditions`) を呼び出す。
            * `live_trading=False`の場合、何もしない（`backtrader`のネイティブ注文に任せる）。
    ...
    * **(新規)** **バックテスト用決済ロジック (`_place_native_exit_orders`)**:
        ...
        * `backtrader`ネイティブの`Order.Limit`（利確）と`Order.StopTrail`（損切り）をOCO注文として発行する。
