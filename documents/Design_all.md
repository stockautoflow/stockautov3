...
1. [**株自動トレードシステム 基本仕様書 (v74.5)**](#bookmark=id.z1wo00mfc041)  
   * [1.1. システムの目的](#bookmark=id.65jicqj2r65f)  
   * [1.2. 主な目標](#bookmark=id.ilhkycp0jatr)  
   * [1.3. システム要件](#bookmark=id.6a53m7rnogps)  
   * [1.4. システム構成](#bookmark=id.2yaggeg6lp2c)  
   * [1.5. 運用フロー](#bookmark=id.l9ntw6uvs33p)  
   * [1.6. リスクと対策](#bookmark=id.adw1t2c1isw1)  
   * [1.7. 免責事項](#bookmark=id.clrdsjacrmkc)  
2. [**株自動トレードシステム 詳細設計書 (v74.5)**](#bookmark=id.1xehpyruz7ym)  
   * [2.1. アーキテクチャ](#bookmark=id.id686gwpc3wq)  
   * [2.2. 環境構築と使用方法](#bookmark=id.8hbb98yrcyh2)  
   * [2.3. モジュール別詳細仕様](#bookmark=id.wlqggd9f4xjd)  
3. [**チャート機能 詳細設計書 (v74.5)**](#bookmark=id.ujx5uag1mgrv)  
   * [3.1. 機能の目的とアーキテクチャ](#bookmark=id.i0s7g1ulrxc1)  
   * [3.2. インターフェース仕様](#bookmark=id.wevr5efh1yio)  
   * [3.3. 機能仕様詳細](#bookmark=id.smrja2ugmqly)  
4. [**ATRトレーリングストップ 実装仕様書 (v74.5)**](#bookmark=id.h0ga67pcbya0)  
   * [4.1. 目的と実装方針](#bookmark=id.2i2a3ms807jw)  
   * [4.2. strategy.yml 仕様](#bookmark=id.2txbdza6mp9e)  
   * [4.3. btrader\_strategy.py 実装仕様](#bookmark=id.g995l39mdxfs)  
5. [**トレード戦略のカスタマイズ方法 (v74.5)**](#bookmark=id.gz55l2wxo3it)  
   * [5.1. データソースの定義 (timeframes)](#bookmark=id.k7anj0qxhss2)  
   * [5.2. エントリー条件の定義 (entry\_conditions)](#bookmark=id.bhm25eektydr)  
   * [5.3. イグジット条件の定義 (exit\_conditions)](#bookmark=id.83res89jnqlt)  
   * [5.4. ポジションサイジングの定義 (sizing)](#bookmark=id.2w1m9ckyzo5i)  
6. [**エントリー戦略アイデア集**](#bookmark=id.9b4v5mqs52tt)
