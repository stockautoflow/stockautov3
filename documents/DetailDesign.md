# **株自動トレードシステム 詳細仕様書 (Backtrader版 v17)**
...
2. chart\_generator.pyが、出力された取引履歴レポートを基に、視覚的なチャート画像を生成する。
...
* **主要な内容**:  
  * **\_\_init\_\_(self)**:  
    ...
    * 取引ごとの情報を保持するためのインスタンス変数（self.order, self.trade\_size, self.entry\_reason）を初期化する。  
  * **next(self)**:  
    * 毎ローソク足ごとに呼び出されるメインロジック。  
    * 長期・中期・短期の3つのエントリー条件がすべて満たされているかを判定する。  
    * 条件が満たされた場合、ATRベースのリスク管理モデルでポジションサイズを計算し、エントリー根拠の文字列を生成・保存した上で、buy\_bracket()でOCO注文を発注する。  
...
* **主要な内容**: generate\_report(...)  
  ...
  * strategy.ymlのパラメータから、レポートに記載する戦略の説明文を動的に生成する。  
...
* **主要な内容**:  
  * **TradeList(bt.Analyzer)**: 取引履歴を詳細に記録するためのカスタムアナライザー。決済理由（利確か損切りか）やエントリー根拠などを収集する。  
  * **run\_backtest\_for\_symbol(...)**:  
    ...
    * TradeAnalyzerとカスタムのTradeListアナライザーを追加する。  
...
* **役割**: バックテスト完了後、取引履歴を視覚化するためのチャート画像を生成する。計算処理から完全に分離されている。  
* **主要な内容**: plot\_charts\_for\_all()  
  * reportディレクトリから最新の取引履歴CSV（trade\_history\_...）を探して読み込む。  
  * 取引があった銘柄ごとにループ処理を行う。  
  * dataディレクトリから対応する価格データCSVを読み込み、チャート描画に必要なテクニカル指標を再計算する。  
  * 取引履歴の日時データを基に、チャート上にプロットする売買マーカーの位置を決定する。  
  * mplfinanceライブラリを使い、ローソク足、出来高、テクニカル指標、売買マーカーを重ね合わせたチャート画像を生成し、chartディレクトリにPNGファイルとして保存する。